...
有些类型是例外，不遵守此规则。如果一个值的类型实现了特殊的 Copy trait，即使它被重新分配到新的内存位置，也不会认为该值已经移动。 相反，该值被复制，并且旧位置和新位置都可以访问。本质上，在移动的目的地构造了具有相同值的另一个相同实例。Rust 中的大多数原始类型，例如整数和浮点类型，都是 Copy。 要成为 Copy，必须可以简单地通过复制它们的bites来复制类型的值。这消除了包含非复制类型的所有类型以及拥有在删除值时必须取消分配的资源的任何类型。

要了解原因，请考虑如果像 Box 这样的类型是 Copy 会发生什么。如果我们执行 box2 = box1，那么 box1 和 box2 都会相信它们拥有分配给 box 的堆内存，并且当它们超出范围时，它们都会尝试释放它。两次释放内存可能会产生灾难性的后果。

当一个值的所有者不再使用它时，所有者有责任通过删除它来对该值进行任何必要的清理。 在 Rust 中，当保存值的变量不再在作用域内时，会自动删除。 类型通常会递归地删除它们包含的值，因此删除复杂类型的变量可能会导致许多值被删除。由于 Rust 的离散所有权要求，我们不会意外多次丢弃相同的值。保存对另一个值的引用的变量不拥有该其他值，因此当变量删除时该值不会删除。
